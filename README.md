...
- **Starting-point presets** — a "Starting points" chip row (shown on both tabs once at least one preset exists) loads a pre-built arrangement generated from a written list of seating preferences. Loading one replaces the current plan (after a confirm) and then normal editing/saving takes over; the preset itself never changes, so you can always come back to it. Presets live in the `PRESETS` constant in `index.html` as ordinary save codes (guest indexes, never names). To add one, give Claude the preference list and the passphrase and have it generate and append the arrangement.
